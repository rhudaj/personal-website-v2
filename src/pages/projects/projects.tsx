...
function ProjectInfo(props: InfoProps) {
  ...
  return (
    <div className="ProjectInfo FullWidth">
      ...
      <span className="closeInfo" onClick={props.closeHandler}>X</span>
    </div>
  );
}
...
export function Projects() {
  ...
  const CreateGrid = (): JSX.Element[] => {
    ...
    const grid = projects_json.map((proj, index)=>{
      if(selected != undefined && index == selected) target_proj = proj;
      return <ProjectThumbnail title={proj[0]} img={proj[1]} n={index} onClick={()=>setSelected(index)}/>
    })

    //Inject the selected <ProjectThumbnail>'s corresponding <ProjectInfo> element
    if(selected != undefined && target_proj != undefined) {
      const element2add = <ProjectInfo title={target_proj[0]} img={target_proj[1]} readme_url={target_proj[3]} closeHandler={closeInfo}/>
